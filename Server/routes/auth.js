...
const {
  saveLoginHistory,
  getLoginHistory,
} = require("../services/loginHistoryService"); // Updated to include getLoginHistory
...
router.post("/login", async (req, res) => {
  ...
  if (browser.toLowerCase().includes("microsoft")) {
    ...
    return res
      .status(200)
      .json({
        message: "Logged in successfully without authentication",
        browser,
      });
  }
  ...
  if (isMobile && (currentTime < 10 || currentTime >= 13)) {
    return res
      .status(403)
      .json({
        message: "Access is restricted to 10 AM - 1 PM on mobile devices",
      });
  }
